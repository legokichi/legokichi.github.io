# cuttleboneを使ってnar配布ページにデモゴを仕込む

この記事は [伺か Advent Calendar 2015](http://www.adventar.org/calendars/1192) の12/24の記事です。

ゴースト開発者向けの記事です。

## 何これ
Webゴースト用シェル描画ライブラリ [cuttlebone](https://github.com/ikagaka/cuttlebone) と
SakuraScript実行ライブラリ [SakuraScriptPlayer.js](https://github.com/ikagaka/SakuraScriptPlayer.js) と
Nar読み込みライブラリ [NarLoader](https://github.com/ikagaka/NarLoader) を利用して
Webゴーストの配布ページでゴーストのデモを動かそうという記事です。

## つかいかた
1. `sample.html` の `<script>var NAR_URL =...` から `ssp.play(shioriRequest({ID:"OnBoot"}));});</script>` までをコピる
2. nar配布ページに貼り付ける
3. `var NAR_URL = "*.nar";` を配布しているnarへのURLにする
4. SakuraScriptを書き換える(SSPのスクリプトログからコピペする)
 
## 注意事項
* SSPのスクリプトログからコピペする場合 `\` を `\\` に置換するようにしましょう
...
<meta charset="utf-8" />
<script>
var NAR_URL = "https://ikagaka.github.io/cuttlebone/nar/mobilemaster.nar";
var BALLOON_URL = "https://ikagaka.github.io/cuttlebone/nar/origin.nar";
var SHELL_ID = "master";
function shioriRequest(req){
  function choice(arr){ return arr[(Math.random()*100*(arr.length)|0)%arr.length]; }
  switch(req.ID){
    case "OnBoot":
      return choice([
        "\\u\\s[10]\\0\\s[401]…\\w5…\\w5…\\w5むあ？\\w9\\w9\\1むあ？\\w9ちゃうわ。\\w9\\n仕事や、\\w5目ぇ覚ませや。\\w9\\w9\\0\\n\\nほんじつのえいぎょーは、\\w9\\w9\\w9\\nしゅーりょーしました。\\w9\\w9\\1\\s[11]\\n\\n\\f[height,20]\\f[bold,1]起きんかいっ！！\\f[bold,default]\\f[height,default]\\e",
        "\\u\\s[10]\\0\\s[26]きゃあああああ！！\\w9\\n何、\\w5なんですかぁ！？\\w9\\w9\\1…\\w5…\\w5仕事やて。\\w9\\n寝ぼけてへんと、\\w5さっさと起きや。\\w9\\w9\\0\\s[3]\\n\\n…\\w5…\\w5ユーザさん…\\w5…\\w5\\w9\\nおはようございます？\\w9\\w9\\1\\n\\nちゃうわい、\\w5ダァホ。\\e"
      ]);
    case "OnTalk":
      return choice([
    ...
    case "OnChoiceSelect":
      switch(req.Reference0){
        case "Choice_AITALK": return shioriRequest({ID:"OnTalk"})
        case "Choice_CONTROL_PANEL": return "\\h\\s[0]その機能はオミットされました。"
        case "Choice_SWISSARMY": return "\\h\\s[0]その機能はオミットされました。"
        case "Choice_SYSTEM_INFO": return "\\h\\s[0]その機能はオミットされました。"
        case "Choice_Gainer": return "\\h\\s[0]その機能はオミットされました。"
        case "Choice_DEBUG": return "\\h\\s[0]その機能はオミットされました。"
        case "Choice_CANCEL": return "\\h\\s[0]はい、\\w5わかりました。\\e"
      }
    case "OnMouseMove":
...
<script src="https://ikagaka.github.io/cuttlebone/dist/cuttlebone.js"></script>
<script src="https://ikagaka.github.io/cuttlebone/bower_components/ikagaka.sakurascriptplayer.js/dist/SakuraScriptPlayer.js"></script>
<script src="https://ikagaka.github.io/cuttlebone/bower_components/narloader/dist/NarLoader.js"></script>
<script>
if(!(NarLoader.loadFromURL instanceof Function) &&
    NarLoader.NarLoader.loadFromURL instanceof Function){
  Object.keys(NarLoader.NarLoader).forEach(function(a){
...
Promise.all([
  ...
  var shellDir = args[0].getDirectory("shell/"+SHELL_ID).asArrayBuffer();
  ...
  named.on("mousedblclick", function(ev){
    ...
    var response = shioriRequest(request);
    ssp.play(response);
  });
  // なでられ
  var moves = {};
  ...
  named.on("mousemove", function(ev){
    ...
    var ID = ""+request.Reference3+request.Reference4;
    moves[ID] = moves[ID] || 0
    moves[ID]++
    clearTimeout(timers[ID])
    timers[ID] = setTimeout(function(){moves[ID]=0}, 5000);
    if (moves[ID] > 100){
      moves[ID] = 0
      var response = shioriRequest(request);
      ssp.play(response);
  ...
  setInterval(function(){
    if (iRandamTalk <= 0){
      ...
      if(!ssp.playing) ssp.play(shioriRequest({ID: "OnTalk"}));
    }else iRandamTalk--;
  }, 1000);
  ssp.play(shioriRequest({ID:"OnBoot"}));
});
</script>
</head>
<body>
